...
st.set_page_config(page_title="Sito Pro v2", layout="wide")
...
inv_iniziale = st.sidebar.number_input('Investimento iniziale (€)', value=100.0, step=50.0)
inv_annuale = st.sidebar.number_input('Investimento annuale (€)', value=10.0, step=10.0)

# --- OPZIONE: LIMITAZIONE INVESTIMENTO ANNUALE ---
st.sidebar.write("L'investimento annuale viene eseguito per tutto il periodo?")
investe_tutto_periodo = st.sidebar.toggle("Sì", value=True)

# Gestione dinamica degli input in base all'interruttore
if investe_tutto_periodo:
    st.sidebar.markdown("<span style='color: #a1a1aa;'>Per quanti anni?</span>", unsafe_allow_html=True)
    anni_contribuzione = st.sidebar.number_input(
    ...
    st.sidebar.markdown("<span style='color: #1e293b; font-weight: bold;'>Per quanti anni?</span>", unsafe_allow_html=True)
    anni_contribuzione = st.sidebar.number_input(
        "Inserisci anni contribuzione", 
        min_value=1, 
        max_value=100, 
...
st.sidebar.markdown("---")
rendimento_perc = st.sidebar.number_input('Rendimento medio annuo (%)', value=7.0, step=0.5)

# --- CONTROLLI ANNI ---
st.write("### Orizzonte Temporale")
...
# --- AREA GRAFICO ---
st.write("---")
c_graph, c_toggle = st.columns([4, 1])
with c_toggle:
...
fig.update_layout(yaxis_type="log" if scala_log else "linear", hovermode="x unified")

# Aggiornato con il nuovo parametro standard width="stretch" per eliminare il warning
st.plotly_chart(fig)
